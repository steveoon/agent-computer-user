...
const agent = new ToolLoopAgent({
  model: deepseek("deepseek-v3.1"), // Default model
  callOptionsSchema: z.object({
...
**Note:** The `prepareCall` function can be async, enabling you to fetch data before configuring the agent.
...
const agent = new ToolLoopAgent({
  ...
  prepareCall: ({ options, ...settings }) => ({
    ...
    model: options.urgency === "high" ? deepseek("deepseek-v3.1") : settings.model,
