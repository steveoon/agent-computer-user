...
const CREDENTIALS = {
  LOGIN_ID: "huangjiahui@duliday.com",
  PASSWORD: "SHVhbmcxMjM=", // Base64编码的密码
} as const;
...
function extractRevenue(order: Order): number {
  ...
  const value = typeof revenue === "string" || typeof revenue === "number"
    ? parseFloat(String(revenue))
    : 0;
...
function parseOrders(chartMain: ChartMain | undefined): Order[] {
  ...
  try {
    ...
    throw new Error(
      `数据解析失败: ${error instanceof Error ? error.message : "未知错误"}`
    );
  }
...
function applyLocalFilters(
  ...
  if (params.regionName) {
    ...
    const needsFilter = filtered.some((o) => {
      ...
      return !(
        typeof region === "string" && region.includes(regionName)
      );
    });
  ...
  if (params.orderRegionName) {
    ...
    const needsFilter = filtered.some((o) => {
      ...
      return !(
        typeof region === "string" && region.includes(orderRegionName)
      );
    });
...
export const dulidayBiReportTool = () =>
  tool({
    ...
    parameters: z.object({
      ...
      orderRegionName: z.string().optional().describe("订单所属地区（细分区域），如：长兴、外高桥、金桥等"),
      sortBy: z
...
function formatSummary(
  ...
  if (params.startDate || params.endDate) {
    message += `   日期范围：${params.startDate || "开始"} ~ ${
      params.endDate || "至今"
    }\n`;
  }
  ...
  if (params.orderRegionName)
    message += `   所属地区：${params.orderRegionName}\n`;
  ...
  Object.entries(storeStats)
    ...
    .slice(0, 5)
    .forEach(([store, stats]) => {
      message += `   ${store}：${stats.count}单，¥${stats.revenue.toFixed(
        2
      )}\n`;
    });
...
function formatDetailed(orders: Order[], totalCount: number): string {
  ...
  ordersToDisplay.forEach((order, index) => {
    ...
    message += `📍 位置：${order[FIELD_NAMES.BIG_REGION]} - ${
      order[FIELD_NAMES.ORDER_REGION]
    }\n`;
  ...
  if (orders.length > LIMITS.MAX_DETAILED_DISPLAY) {
    ...
    message += `📊 剩余 ${
      orders.length - LIMITS.MAX_DETAILED_DISPLAY
    } 条记录未显示\n`;
  }
...
function formatNotification(orders: Order[]): string {
  ...
  const todayOrders = orders.filter(
    (o) => o[FIELD_NAMES.ORDER_DATE] === today
  );
  const pendingOrders = orders.filter(
    (o) => o[FIELD_NAMES.ORDER_STATUS] === "待接受"
  );

  if (todayOrders.length > 0) {
    message += `📅 今日订单（${today}）：\n`;
    todayOrders.forEach((order) => {
      message += `• ${order[FIELD_NAMES.STORE_NAME]} - ${
        order[FIELD_NAMES.SERVICE_DATE]
      }\n`;
  ...
  if (pendingOrders.length > 0) {
    ...
    pendingOrders
      .slice(0, LIMITS.MAX_NOTIFICATION_PENDING)
      .forEach((order) => {
        message += `• ${order[FIELD_NAMES.STORE_NAME]} - ${
          order[FIELD_NAMES.ORDER_DATE]
        }\n`;
