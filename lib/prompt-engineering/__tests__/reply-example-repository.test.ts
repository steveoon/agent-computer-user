...
describe("ReplyExampleRepository - 完整功能测试", () => {
  ...
  describe("getSimilarExamples 方法 (对应原getRelevantExamples)", () => {
    it("应该返回相关的示例", () => {
      // 测试有完整上下文信息的情况
      const examplesWithContext = exampleRepo.getSimilarExamples(
        "有什么兼职工作吗？",
        "initial_inquiry",
        2,
        "徐汇区肯德基有岗位"
      ...
      const examplesWithoutContext = exampleRepo.getSimilarExamples(
        "有工作吗？",
        "initial_inquiry",
        2,
        ""
    ...
    it("应该根据上下文智能选择示例", () => {
      ...
      // 检查是否选择了追问类型的示例
      const hasQuestionExample = genericExamples.some(
        (ex: any) => ex.output.includes("你想了解哪个岗位") || ex.output.includes("不同岗位")
      );
  ...
  describe("getSimilarExamples 方法", () => {
    it("应该根据相似度返回示例", () => {
      const examples = exampleRepo.getSimilarExamples("工资多少钱？", "salary_inquiry", 1);
    ...
    it("应该处理不存在的场景类型", () => {
      const examples = exampleRepo.getSimilarExamples("测试消息", "non_existent_type", 2);
    ...
    it("应该按相似度排序返回多个示例", () => {
      const examples = exampleRepo.getSimilarExamples("在哪里工作", "location_inquiry", 3);
      ...
      if (examples.length > 1) {
        const firstRelevance = (exampleRepo as any).calculateSimilarity(
          "在哪里工作",
          examples[0].input
        );
        const secondRelevance = (exampleRepo as any).calculateSimilarity(
          "在哪里工作",
          examples[1].input
        );
  ...
  describe("场景覆盖性测试", () => {
    ...
    it("应该区分通用示例和具体示例", () => {
      ...
      const hasGenericExample = genericExamples.some(
        (ex: any) => !ex.output.match(/\d+元/) // 不包含具体金额
      );
      expect(hasGenericExample).toBe(true);

      // 当有完整上下文时，可以返回具体示例
      const specificExamples = exampleRepo.getSimilarExamples(
        "工资多少？",
        "salary_inquiry",
        2,
        "肯德基服务员，22-25元/时"
      );
      const hasSpecificExample = specificExamples.some(
        (ex: any) => ex.output.match(/\d+元/) // 包含具体金额
      );
