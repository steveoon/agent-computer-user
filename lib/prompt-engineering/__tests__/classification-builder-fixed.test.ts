  ...
  describe('核心功能测试', () => {
    describe('buildAtomicSystemPrompt', () => {
      ...
      it('应该包含所有必要的约束条件', () => {
        ...
        expect(atomicPrompt.constraints).toContain('对敏感话题（年龄、保险、身体条件）保持高度敏感');
    ...
    describe('getRelevantExamples', () => {
      it('应该返回相关的分类示例', () => {
        const examples = classificationBuilder.getRelevantExamples(
          '工资多少？',
          {},
          3
        );
      ...
      it('应该基于消息内容返回合适的示例', () => {
        const salaryExamples = classificationBuilder.getRelevantExamples(
          '工资待遇如何',
          {},
          2
        );

        const locationExamples = classificationBuilder.getRelevantExamples(
          '在哪里上班',
          {},
          2
        );
