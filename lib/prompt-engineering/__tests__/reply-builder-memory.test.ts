  ...
  describe('CellularMemoryManager 完整功能', () => {
    ...
    describe('longTermMemory 长期记忆测试', () => {
      ...
      it('应该智能提取真实对话中的品牌信息', () => {
        ...
        let brandEntries = Array.from(longTermMemory.entries()).filter(([key]) => key.startsWith('brand_'));
        ...
        brandEntries = Array.from(longTermMemory.entries()).filter(([key]) => key.startsWith('brand_'));
    ...
    it('应该在超出token预算时优化内存', () => {
      const longHistory = Array(100).fill(null).map((_, i) => 
        `用户: 这是第${i}条消息，包含很多很多很多很多很多很多很多很多很多很多内容`
      );
    ...
    it('应该清理过期内存', () => {
      ...
      // 长期记忆应该被限制在合理范围内
      const totalFacts = Object.values(context.facts).reduce((sum, arr) => 
    ...
    describe('三种内存类型交互关系测试', () => {
      ...
      it('应该支持内存重置后的完全清理', () => {
        ...
        expect(stats.shortTermCount + stats.workingMemoryCount + stats.longTermCount).toBeGreaterThan(0);
